...
namespace NxDesk.Infrastructure.Services
{
    public class SIPSorceryWebRTCService : IWebRTCService
    {
        ...
        {
            System.Diagnostics.Trace.WriteLine($"[CLIENT] Iniciando conexi�n a host: {hostId}");
            OnConnectionStateChanged?.Invoke("Conectando...");
            
            // Asegurar que estamos suscritos al evento de señalización (evitar duplicados)
            _signalingService.OnMessageReceived -= HandleSignalingMessage;
            _signalingService.OnMessageReceived += HandleSignalingMessage;
            ...
            if (!await _signalingService.ConnectAsync(hostId))
            {
                ...
                OnConnectionStateChanged?.Invoke("Error de se�alizaci�n");
                return false;
            }
            ...
            var config = new RTCConfiguration
            {
                ...
                { 
                    // M�ltiples servidores STUN para mejor conectividad
            ...
            _pc.OnVideoFrameReceived += (endpoint, timestamp, frame, format) =>
            {
                ...
                try
                {
                    ...
                    {
                        System.Diagnostics.Trace.WriteLine("[CLIENT] Frame vac�o recibido");
                        return;
                    }
                    ...
                    if (rawSamples == null)
                    {
                        System.Diagnostics.Trace.WriteLine("[CLIENT] Decoder retorn� null");
                        return;
                    }
